...
class DecisionTree:
    def __init__(self, criterion="gini", max_depth=10, min_samples_split=2, n_features=None):
        """
        ...
        - n_features: Number of features to consider for splits (use all if None).
        """
    ...
    def _information_gain(self, labels, feature_values, split_threshold):

        # Impurity of the parent node (before the split)
        parent_impurity = self._impurity(labels)

        # Split data based on the threshold
        left_indices, right_indices = self._split(feature_values, split_threshold)
        ...
        left_impurity = self._impurity(labels[left_indices])
        right_impurity = self._impurity(labels[right_indices])
    ...
    def _impurity(self, y):
        """
        Calculate node impurity based on the chosen criterion.
        """
        ...
        if self.criterion == "gini":
            ...
            raise ValueError(f"Invalid criterion '{self.criterion}'. Choose 'gini', 'entropy', or 'error'.")
...
# Main block for testing the decision tree
if __name__ == "__main__":
    # Load dataset
    file_path = 'Dataset\\Telco-Customer-Churn.csv'  # Adjust path if needed
    churn_data = pd.read_csv(file_path)

    # Preprocess data
    churn_data = churn_data.drop(['customerID'], axis=1)
    churn_data['TotalCharges'] = churn_data.apply(
        lambda row: row['tenure'] * row['MonthlyCharges'] if pd.isnull(row['TotalCharges']) else row['TotalCharges'], axis=1
    )
    for column in churn_data.select_dtypes(include=['object']).columns:
        churn_data[column] = churn_data[column].astype('category').cat.codes
    ...
    # Example usage
    tree = DecisionTree(criterion="gini", max_depth=5, min_samples_split=10)
